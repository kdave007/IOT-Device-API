const alertsSetUp = require("../models/mail.alerts.model");
...
exports.tempsWatchdog = async() => {
     const user = new User({
          email: null,
          pass: null
     });
     
     const from = '2020-01-01 00:00:00';//testing purpose only
              ...
              if(gotAlerts(aSettings.err,uList.data[index].userId)){
                 ...
                 for(let i=0;i<aSettings.data.length;i++){
                    ...
                    if(aSettings.data[i].alertType<=4){
                         ...
                         let temps = await tempsProvider.getSortedbyTS(aSettings.data[i].deviceId,fom,tu);//retrieve temps for every deviceId found in the alerts setup
                        // console.table(temps);
